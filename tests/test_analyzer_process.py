...
from csst.analyzer.process import (
    reactor_transmission_at_temp,
    reactor_transmission_at_temps,
)
from csst.experiment.models import PropertyValues, PropertyValue
from .fixtures.data import reactor


def test_reactor_transmission_at_temp(reactor):
    """Tests processing of transmission. See reactor fixture for data"""
    ptrans = reactor_transmission_at_temp(reactor, temp=5)
    assert ptrans.average == 4.5
    assert ptrans.median == 4.5
    assert ptrans.reactor == reactor
    assert round(ptrans.std, 3) == 0.5

    ptrans = reactor_transmission_at_temp(reactor, temp=5, temp_range=10)
    assert ptrans.average == 4.5
    assert ptrans.median == 4.5
    assert ptrans.reactor == reactor
    assert round(ptrans.std, 3) == 0.5

    ptrans = reactor_transmission_at_temp(reactor, temp=5, temp_range=11)
    assert ptrans.average == 12.75
    assert ptrans.median == 12.5
    assert ptrans.reactor == reactor
    assert round(ptrans.std, 3) == 8.288

    ptrans = reactor_transmission_at_temp(reactor, temp=15, temp_range=11)
    assert ptrans.average == 56.5
    assert ptrans.median == 64
    assert ptrans.reactor == reactor
    assert round(ptrans.std, 3) == 24.187

    assert reactor_transmission_at_temp(reactor, temp=0) is None


def test_reactor_transmission_at_temps(reactor):
    # reactor values sorted by temp would be
    # temp = [5, 5, 10, 10, 15, 15, 20, 20, 20, 20]
    # trans = [5, 4, 20, 22, 50, 45, 78, 78, 79, 80]
    ptransmissions = reactor_transmission_at_temps(reactor, [5, 10, 15, 20])
    ...
    ptransmissions = reactor_transmission_at_temps(
